            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form v-on:submit.prevent="onSubmit">
                        <div class="row mb-3">
                            <div class="col text-secondary font-weight-bold title-line">{{ langs.section.info }}</div>
                        </div>
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                <label :for="`${modalId}-arrived-at`">
                                    {{ langs.fields.arrived_at }}
                                    <span class="text-danger">*</span>
                                </label>
                                <datepicker
                                    format="yyyy-MM-dd"
                                    ...
                                    :placeholder="langs.fields.arrived_at"
                                ></datepicker>
                            ...
                            <div class="form-group col-md-6">
                                <label :for="`${modalId}-address`">
                                    {{ langs.fields.address }}
                                    <span class="text-danger">*</span>
                                </label>
                                <input
                        ...
                        <div class="form-group">
                            ...
                            <textarea
                                ...
                                v-model="package.remark"
                            ></textarea>
                        </div>

                        <div class="row mb-3">
                            <div class="col text-secondary font-weight-bold title-line">{{ langs.section.content }}</div>
                        </div>
                        ...
                        <div
                            class="form-row py-2"
                            :class="{'bg-light-primary': caseIndex%2==0 }"
                            v-for="(caseItem, caseIndex) in package.cases"
                            :key="caseIndex"
                        >
                            <div class="col-md-6">
                                <select
                                    class="form-control"
                                    v-model="caseItem.case_id"
                                    :class="{'is-invalid': hasCaseError(caseIndex, 'case_id')}"
                                    required
                                >
                                    <option value hidden>{{langs.placeholder.cases}}*</option>
                                    <option
                                        v-for="(option, optionIndex) in caseList"
                                        :key="optionIndex"
                                        :value="option.id"
                                    >{{ option.case_type_name }}</option>
                                </select>
                                <div class="invalid-feedback">
                                    <div
                                        v-for="(msg, msgIndex) in getCaseError(caseIndex, 'case_id')"
                                        :key="msgIndex"
                                    >{{msg}}</div>
                                </div>
                            </div>
                                ...
                                <div class="input-group">
                                    <div class="input-group-prepend cursor-pointer">
                                        <button
                                            ...
                                            type="button"
                                            v-on:click="caseItem.amount-=1"
                                            :disabled="caseItem.amount==1"
                                        >
                                    ...
                                    <div class="input-group-append cursor-pointer">
                                        <button class="btn btn-primary" type="button" v-on:click="caseItem.amount+=1">
                            ...
                            <div class="col-md-2">
                                <button
                                    v-on:click="delCase(caseIndex)"
                                    type="button"
                                    class="col btn btn-secondary"
                                >{{ langs.functional.del_content }}</button>
                            </div>
                        </div>
                        <div class="form-row">
                            <div class="col-12">
                                <button
                                    v-on:click="addCase()"
                                    type="button"
                                    class="col btn btn-primary rounded-0"
                                >+ {{ langs.functional.add_content }}</button>
                            </div>
                        </div>
                    </form>
                </div>
                <div class="modal-footer">
                    ...
                    >{{langs.functional.cancel}}</button>
